...
* ``hipace.deposit_temp`` (`bool`) optional (default `0`)
    If the weight, momentum (``ux``, ``uy``, ``uz``) and their squares (``ux^2``, ``uy^2``, ``uz^2``) of the plasma should be deposited so that it is available as a diagnostic.
...
* ``hipace.deposit_temp_individual`` (`bool`) optional (default `0`)
    This option works similar to ``hipace.deposit_temp``,
    however the weights, momentum and their squares from every plasma species will be deposited into individual fields
    that are accessible as ``ux_<plasma name>`` or ``ux^2_<plasma name>`` in ``diagnostic.field_data``.
