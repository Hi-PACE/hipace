...
def main():
    ...
    p2 = raw.get('p2')
    p3 = raw.get('p3')
    q =  raw.get('q') # * self.cellvol

    if (args.q_beam and args.n0):
        print("Renormalizing beam..")
        sum_of_weights = np.sum(q)
        q = len(q) * q / sum_of_weights

        Vcell_in_meter_cubed = args.q_beam/ ( args.n0 * constants.e * len(q) )

        plasma_wavenumber_in_per_meter = np.sqrt(args.n0 * constants.e**2 / \
            ( constants.m_e * constants.epsilon_0 ) )/ constants.c

        Vcell = plasma_wavenumber_in_per_meter**3 * Vcell_in_meter_cubed
    ...
    particel["r"]["x"].reset_dataset(dataset)
    particel["r"]["x"].store_chunk(x3)
    particel["r"]["y"].reset_dataset(dataset)
    particel["r"]["y"].store_chunk(x2)
    particel["r"]["z"].reset_dataset(dataset)
    particel["r"]["z"].store_chunk(x1)
    if (args.q_beam and args.n0):
        particel["r"]["x"].unit_SI = 1/plasma_wavenumber_in_per_meter
        particel["r"]["y"].unit_SI = 1/plasma_wavenumber_in_per_meter
        particel["r"]["z"].unit_SI = 1/plasma_wavenumber_in_per_meter

    # same for the momentum!
    particel["u"]["x"].reset_dataset(dataset)
    particel["u"]["x"].store_chunk(p3)
    #particel["u"]["x"].unit_SI = 1
    particel["u"]["y"].reset_dataset(dataset)
    particel["u"]["y"].store_chunk(p2)
    #particel["u"]["y"].unit_SI = 1
    particel["u"]["z"].reset_dataset(dataset)
    particel["u"]["z"].store_chunk(p1)
    #particel["u"]["z"].unit_SI = 1

    particel["q"]["q"].reset_dataset(dataset)
    particel["q"]["q"].store_chunk(q) #*constants.e)
    # print(np.mean(q))
    # print(np.std(q))
    ...
    if (args.q_beam and args.n0):
        particel["q"]["q"].unit_SI = constants.e * args.n0 /(plasma_wavenumber_in_per_meter**3)
    else:
        particel["q"]["q"].unit_SI = constants.e

    particel["m"]["m"].reset_dataset(dataset)
    particel["m"]["m"].store_chunk(q*constants.m_e)
    if (args.q_beam and args.n0):
        particel["m"]["m"].unit_SI = constants.m_e * args.n0 /(plasma_wavenumber_in_per_meter**3)
    else:
        particel["m"]["m"].unit_SI = constants.m_e
