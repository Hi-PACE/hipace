...
Subscript $p$ means plasma, subscript $b$ means beam. Brackets in superscript $(n+2)$ mean "predicted". $\rho$ and capital letters are field data, others are particle data. $x$, $u$ and $\psi$ are the position, momentum and pseudo-potential respectively.
...
\end{align}
\text{And then, while $error(B_{\perp, iter}^{(n+1)}, B_{\perp}^{(n+1)})$:}
\begin{align}
