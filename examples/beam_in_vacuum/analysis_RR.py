...
ir_with_RR = diag.InSituReader('diags/insitu/reduced_beam.*.txt')

ne = 5e24
wp = np.sqrt(ne*scc.e**2 / (scc.m_e * scc.epsilon_0 ))
kp = wp/scc.c

mean_gamma0 = ir_with_RR.avg_data("gamma_mean")[0] # should be 2000
std_gamma0 = ir_with_RR.avg_data("gamma_spread")[0] \
            /ir_with_RR.avg_data("gamma_mean")[0] # should be 0.01
epsilonx0 = ir_with_RR.avg_data("emittance_x")[0] # sigma_x0**2 *np.sqrt(mean_gamma0/2) *kp
# should be 313e-6

# final simulation values
mean_gamma_sim = ir_with_RR.avg_data("gamma_mean")[-1]
std_gamma_sim = ir_with_RR.avg_data("gamma_spread")[-1] \
            /ir_with_RR.avg_data("gamma_mean")[-1]
epsilonx_sim = ir_with_RR.avg_data("emittance_x")[-1]
